...
from common.utils.geometry_utils import xyt_global_to_base
from common.utils.spot import angular_distance_from_angle
...
class DiscretePlanner:
    ...
    def plan(
        ...
        found_goal: bool,
        goal_pose: float = None, # type: ignore
        debug: bool = False,
        use_dilation_for_stg: bool = False,
        timestep: int = None, # type: ignore
    ) -> Tuple[DiscreteNavigationAction, np.ndarray, Tuple[int,int], np.ndarray, bool, bool]:
        ...
        self.visited_map[gx1:gx2, gy1:gy2][ # type: ignore
        ...
        return action, closest_goal_map, short_term_goal, dilated_obstacles, False, stop
    ...
    def _get_short_term_goal(
        ...
        # Dilate obstacles
        dilated_obstacles = cv2.dilate(obstacles, self.obs_dilation_selem, iterations=1) # type: ignore

        # Create inverse map of obstacles - this is territory we assume is traversible
        # Traversible is now the map
        traversible = 1 - dilated_obstacles # type: ignore
        traversible[self.collision_map[gx1:gx2, gy1:gy2][x1:x2, y1:y2] == 1] = 0 # type: ignore
        traversible[self.visited_map[gx1:gx2, gy1:gy2][x1:x2, y1:y2] == 1] = 1 # type: ignore
