...
from common.env_utils.sense import AgentPoseSense, BBSense, DepthSense

log = logging.getLogger(__name__)


def solve_inconsistency(
    update_vox_ids, object_id_to_logits, object_ids, voxel_ids, solve_function
)-> list:
    results = []
    for update_vox_id in update_vox_ids:
        objects = object_ids[voxel_ids == update_vox_id]
        if len(objects) == 0:
            continue
        ...
        resolved_class, resolved_logits = solve_function(logits)
        results.append((update_vox_id, (resolved_class, resolved_logits)))

    return results


def _seal_impl(logits) -> tuple:
...
def _ours_bayesian(logits) -> tuple:
    if len(logits.shape) == 1:
        resolved_class = logits.argmax()
        resolved_logits = logits
    else:
...
class SemanticPointCloud:
    ...
    def update_logits(self, instances: Instances, infos) -> None:
    ...
    @staticmethod
    def _get_mesh_from_pcd(_pcd) -> o3d.geometry.TriangleMesh:
    ...
    def preprocess(self, lower_bound=None, upper_bound=None):
        ...
        consistency_solution = partial(
            solve_inconsistency,  # main inconsistency solution function
        ...
        update_voxels_ids = np.setdiff1d(voxel_ids, self.current_voxel_ids)
        self.current_voxel_ids = np.unique(voxel_ids)
        cpu_counts = mp.cpu_count()
        n_chunks = len(update_voxels_ids) // cpu_counts
        chunks = [
            update_voxels_ids[
                i * n_chunks : min(i * n_chunks + n_chunks, len(update_voxels_ids))
            ]
        ...
        with mp.Pool(cpu_counts) as pool:
            solutions = pool.map(consistency_solution, chunks)
