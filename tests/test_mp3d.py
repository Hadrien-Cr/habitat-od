"""
Sanity check that the MP3D scene dataset (see INSTALL.MD) loads correctly in habitat_sim:
instantiates one of the standard MP3D ObjectNav/PointNav val scenes directly via raw
habitat_sim.Simulator (no habitat-lab Env/task) and renders an rgb+semantic overlay, labeled
with each visible instance's category name and its 3D oriented-bounding-box wireframe, at
random navigable positions - dumped as plain-rgb|overlay side-by-side frames to
tests/testdump/test_mp3d/ for visual inspection.
...
same as Gibson-Semantic (see test_gibson.py). Boxes come from that same object's .obb, projected
as pure 3D geometry with no occlusion culling, so a box can extend behind whatever's actually
...
_DATASET_CONFIG = (
    f"{HABITAT_DATA}/scene_datasets/mp3d/mp3d.scene_dataset_config.json"
...
# Standard MP3D ObjectNav/PointNav val split; any one of the eleven works as a smoke test.
_VAL_SCENES = [
    "2azQ1b91cZZ", "8194nk5LbLH", "EU6Fwq7SyZv", "oLBMNvg9in8", "pLe4wQe7qrG",
    "QUCTc6BB5sX", "TbHJrupSAjP", "X7HyMhZNoso", "x8F5xyUWy9e", "YFuZgdQ5vWj",
    "Z6MFQCViBuw", "zsNo4HB9uLZ",
]
...
def _draw_obb_wireframe(draw, obb, color, cam_pos, forward, right, up, focal):
    ...
    for corner in _BOX_CORNERS_LOCAL:
        ...
        world_corners.append(np.array([c.x, c.y, c.z]))
...
def test_generates_rgb_semantic_overlay_frames(sim):
    if not sim.pathfinder.is_loaded:
        pytest.skip(f"no navmesh loaded for {_SCENE} - check the mp3d assets")

    objects_by_id = _objects_by_id(sim)
    assert objects_by_id, (
        f"{_SCENE} loaded with no semantic annotations - check mp3d.scene_dataset_config.json"
    )
