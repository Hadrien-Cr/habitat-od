...
--config points at a habitat_embodied_al/pretrain/config/ds.yaml: run_name
(keys datasets/<run_name>/) + object_params (vocab) + train_scenes/
val_scenes/steps_per_episode/trainer_name/filter_empty (collection
params).
...
collection time, not reusable across a different ds.yaml anyway.

Usage:
  PYTHONPATH=. python habitat_embodied_al/collect_dataset.py \
      --config habitat_embodied_al/pretrain/config/ds.yaml
"""
import argparse
import shutil
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="habitat_embodied_al/pretrain/config/ds.yaml")
    parser.add_argument(
        "--keep-raw",
        action="store_true",
    ...
    for split_name, scenes in [("train", ds_cfg.train_scenes), ("val", ds_cfg.val_scenes)]:
